...
CLI 실행(`main.py`)과 Streamlit 대시보드(`streamlit_app.py`)를 모두 지원합니다.

## 1. 프로젝트 개요

- 게임 루프: `setup -> night -> day -> vote -> night ...`
- 핵심 실행 함수: `app/runner/single_match.py`의 `run_single_match()`
- 주요 산출물:
  - 이벤트 로그(`events.jsonl`)
  - 요약 파일(`summary.json`)
  - 메트릭 리포트(표준 출력)
...
구현의 중심은 `app/runner/single_match.py`의 `_append_day_phase_talk()`와 `app/runner/speech_queue.py`입니다.
...
- 매치 러너: `app/runner/single_match.py`
- 발언 큐: `app/runner/speech_queue.py`
- 페이즈 전이: `app/engine/phase.py`
- 승리/야간 판정: `app/engine/rules.py`
- 투표 판정: `app/engine/vote.py`
- 설정 로더/검증: `config.py`
